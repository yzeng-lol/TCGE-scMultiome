...
#sample_aggr   = args[1]
out_dir = paste0(getwd(), "/integration/atac")         ## no forward slash at the end!!
...
###########################################################################
{
  ...
  ##  UMAPs for merged, harmonized and integrated RNA
  {
    ...
    ggsave(paste0(out_dir, "/Merged_Harmonized_Anchored_ATAC_UMAPs_grouped_by_sample.pdf"), width = 12, height = 4)
    ...
    g <- p1 + p2 + p3  & theme(plot.title = element_text(hjust = 0.5))
    ggsave(paste0(out_dir, "/Merged_Harmonized_Anchored_ATAC_UMAPs_grouped_by_cluster.pdf"), width = 15, height = 4)
  }
}
